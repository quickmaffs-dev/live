...
import {findGradient, getEquation} from './coordinate_geometry_exercises';

class ex04_linear_equations extends React.Component {
    constructor(props) {
        super(props);
        this.x = 1;
        this.Ans = 1;
    ...
    makeQuestion = () => { 
        ...
        for (let i = 0; i < document.querySelectorAll(".mcAnsBtn").length; i++) {            
            document.querySelectorAll(".mcAnsBtn")[i].onclick = () => {checkAns(correctAns, document.querySelectorAll(".mcAnsBtn")[i].innerHTML, this.question_string, "coordinate geometry ex04")};
        }
        this.writeFormula();
        this.writeExample();
    }
...
export default ex04_linear_equations;