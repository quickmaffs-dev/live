...
import {calculateWeeklyWageByHour, calculateAnnualSalary, calculateWeeklyWageBySalary, hourlyPayRate} from './earning_money_exercises';

class ex01_wages_and_salaries extends React.Component {
    constructor(props) {
        super(props);
        this.x = 1;
        this.Ans = 1;
    ...
    componentDidMount(){
        ...
        document.querySelectorAll(".topicSubHeading")[0].innerHTML = "This will ask questions on wages and salaries. Click the Start button below to begin...";            
    ...
    makeQuestion = () => { 
        ...
        if (chooseQuestion === 1) {
            result = calculateWeeklyWageByHour();
        } else if (chooseQuestion === 2) {
            result = calculateAnnualSalary();
        } else if (chooseQuestion === 3) {
            result = calculateWeeklyWageBySalary();
        } else if (chooseQuestion === 4) {
            result = hourlyPayRate();            
        } else {
        ...
        for (let i = 0; i < document.querySelectorAll(".mcAnsBtn").length; i++) {            
            document.querySelectorAll(".mcAnsBtn")[i].onclick = () => {checkAns(correctAns, document.querySelectorAll(".mcAnsBtn")[i].innerHTML, this.question_string, "earning money ex01")};            
        }
...
export default ex01_wages_and_salaries;