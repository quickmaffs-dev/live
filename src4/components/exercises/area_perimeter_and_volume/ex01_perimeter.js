...
import {perimetersBasicShapes} from './area_perimeter_and_volume_exercises';

class ex01_perimeter extends React.Component {
    constructor(props) {
        super(props);
        this.x = 1;
        this.Ans = 1;
    ...
    componentDidMount(){
        ...
        document.querySelectorAll(".topicSubHeading")[0].innerHTML = "This will ask quesitons regarding perimeter. Click the Start button below to begin...";
    ...
    makeQuestion = () => { 
        ...
        //chooseQuestion = 2;
        if (chooseQuestion === 1) {
            result = perimetersBasicShapes();
        //} else if (chooseQuestion === 2) {
            //mcOptions = this.perimeterCompositeShapes();
        } else {
        ...
        for (let i = 0; i < document.querySelectorAll(".mcAnsBtn").length; i++) {            
            document.querySelectorAll(".mcAnsBtn")[i].onclick = () => {checkAns(correctAns, document.querySelectorAll(".mcAnsBtn")[i].innerHTML, this.question_string, "area, perimeter and volume ex01")};
        }
        this.writeFormula();
        this.writeExample();
    }
